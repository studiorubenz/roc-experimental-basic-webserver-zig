...
host is a remarkably good fit: all ABI types (`RocStr`, typed `RocList`,
tag-union layouts with compile-time size asserts) are **generated from
the platform's own API by `roc glue`**
into the checked-in
[`platform/roc_platform_abi.zig`](platform/roc_platform_abi.zig) — no FFI
re-declarations, no `roc_std` layer, no dependency on the compiler's
source tree at build time, and one Zig toolchain builds everything. Same
goal (minimise friction), opposite starting point, opposite conclusion.
...
- The Roc compiler **built from source at commit `05d7069072`** (the
  host symbol ABI, roc-lang/roc#9613 + #9609), checked out as a sibling
  directory of this repo named `roc-05d70690` (`build.sh` and `glue.sh`
  look for the `roc` binary there; `zig build` itself needs no compiler
  checkout):

```sh
git clone https://github.com/roc-lang/roc roc-05d70690
cd roc-05d70690 && git checkout 05d7069072 && zig build
```
...
```
┌─────────┐  accept   ┌──────────────┐  parse HTTP   ┌─────────────────┐
│ acceptor │ ───────▶ │ worker pool  │ ────────────▶ │ roc_handle      │
│ (poll)   │          │ (≤16 threads)│  Roc records  │ (parallel, one  │
└─────────┘          └──────────────┘ ◀──────────── │  per worker)    │
                            │            Response    └─────────────────┘
                            ▼
                     WebSocket frames ⇆ roc_ws_message + broadcast registry
```

The host (`platform/host.zig`, ~1800 lines) implements every effect with
...
Hosted functions use the host symbol ABI (roc-lang/roc#9613): each is a
plain C-ABI function exported under the symbol name declared in
`main.roc`'s `hosted` section — arguments by value, return by value, no
RocOps parameter, no dispatch table — and compiled Roc code calls those
symbols directly. Unused effects are dead-code-eliminated from the final
binary by the linker.

Everything that crosses the Roc↔host boundary is typed by
`platform/roc_platform_abi.zig`, generated by the compiler's `roc glue`
subcommand with its bundled `ZigGlue.roc` spec. The generated file is
self-contained (its own `RocStr`/`RocList`/`RocOps` implementations) and
carries `comptime` size/alignment asserts for every record and tag union,
so ABI drift after a compiler upgrade becomes a compile error.

### Regenerating glue

When the platform API changes (adding or modifying hosted functions in
`platform/*.roc`), regenerate the bindings and rebuild:

```sh
./glue.sh          # runs: roc glue <roc-src>/src/glue/src/ZigGlue.roc platform/ platform/main.roc
```

One known quirk at the pinned commit: the glue generator deduplicates tag
unions by *name*, so all `Try(ok, err)` instantiations collapse into a
single `abi.Try` type and the hosted-function signatures in
`PlatformHostedFns` nominally use it for every fallible return. The actual
return-slot layouts differ per function — the host therefore keeps writing
return values through documented byte offsets (verified by the probe
platform, see [IO-PORT-PLAN.md](IO-PORT-PLAN.md)) and adapts the table
entries with `@ptrCast`.
