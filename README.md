...
the hosted-function dispatch table, tag-union layouts with compile-time
size asserts) are **generated from the platform's own API by `roc glue`**
into the checked-in
...
- The Roc compiler **built from source at commit `48b28c07`**, checked out
  as a sibling directory of this repo named `roc-48b28c07` (`build.sh` and
  `glue.sh` look for the `roc` binary there; `zig build` itself needs no
  compiler checkout):
