...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(Cmd) != 80) @compileError("Cmd size mismatch");
        if (@alignOf(Cmd) != 8) @compileError("Cmd alignment mismatch");
    }
}

/// Element type for __AnonStruct9
pub const __AnonStruct9 = extern struct {
    @"stderr_bytes": RocListWith(u8, false),
    @"stdout_bytes": RocListWith(u8, false),
    @"exit_code": i32,
};

comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(__AnonStruct9) != 56) @compileError("__AnonStruct9 size mismatch");
        if (@alignOf(__AnonStruct9) != 8) @compileError("__AnonStruct9 alignment mismatch");
    }
}
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(__AnonStruct12) != 48) @compileError("__AnonStruct12 size mismatch");
        if (@alignOf(__AnonStruct12) != 8) @compileError("__AnonStruct12 alignment mismatch");
    }
}
...
pub const Request = extern struct {
    ...
    @"headers": RocList(__AnonStruct58),
    @"method": Method,
    @"timeout_ms": NoTimeoutOrTimeoutMilliseconds,
    @"uri": RocStr,
};

comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(Request) != 120) @compileError("Request size mismatch");
        if (@alignOf(Request) != 8) @compileError("Request alignment mismatch");
    }
}
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(Response) != 56) @compileError("Response size mismatch");
        if (@alignOf(Response) != 8) @compileError("Response alignment mismatch");
    }
}
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(__AnonStruct66) != 48) @compileError("__AnonStruct66 size mismatch");
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(__AnonStruct69) != 96) @compileError("__AnonStruct69 size mismatch");
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(IOErr) != 32) @compileError("IOErr size mismatch");
        if (@alignOf(IOErr) != 8) @compileError("IOErr alignment mismatch");
    }
}

/// Tag discriminant for Method.
pub const MethodTag = enum(u8) {
    CONNECT = 0,
    DELETE = 1,
    GET = 2,
    HEAD = 3,
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(Method) != 32) @compileError("Method size mismatch");
        if (@alignOf(Method) != 8) @compileError("Method alignment mismatch");
    }
}

/// Tag discriminant for NoTimeoutOrTimeoutMilliseconds.
pub const NoTimeoutOrTimeoutMillisecondsTag = enum(u8) {
    NoTimeout = 0,
    TimeoutMilliseconds = 1,
};

/// Tag union: NoTimeoutOrTimeoutMilliseconds
pub const NoTimeoutOrTimeoutMilliseconds = extern struct {
    payload: extern union {
        no_timeout: [0]u8,
        timeout_milliseconds: u64,
    },
    tag: NoTimeoutOrTimeoutMillisecondsTag,
};

comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(NoTimeoutOrTimeoutMilliseconds) != 16) @compileError("NoTimeoutOrTimeoutMilliseconds size mismatch");
        if (@alignOf(NoTimeoutOrTimeoutMilliseconds) != 8) @compileError("NoTimeoutOrTimeoutMilliseconds alignment mismatch");
    }
}
...
pub const CmdHost_exec_exit_codeArgs = extern struct {
    @"args": RocList(RocStr),
    @"envs": RocList(RocStr),
    @"program": RocStr,
    @"clear_envs": bool,
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(CmdHost_exec_exit_codeArgs) != 80) @compileError("CmdHost_exec_exit_codeArgs size mismatch");
        if (@alignOf(CmdHost_exec_exit_codeArgs) != 8) @compileError("CmdHost_exec_exit_codeArgs alignment mismatch");
    }
}
...
/// Refcounted fields are owned by the hosted function.
pub const CmdHost_exec_outputArgs = extern struct {
    @"args": RocList(RocStr),
    @"envs": RocList(RocStr),
    @"program": RocStr,
    @"clear_envs": bool,
...
comptime {
    if (@sizeOf(usize) == 8) {
        if (@sizeOf(CmdHost_exec_outputArgs) != 80) @compileError("CmdHost_exec_outputArgs size mismatch");
        if (@alignOf(CmdHost_exec_outputArgs) != 8) @compileError("CmdHost_exec_outputArgs alignment mismatch");
    }
}
...
/// Refcounted fields are owned by the hosted function.
pub const DirCreateArgs = extern struct {
    arg0: RocStr,
};
...
/// Roc signature: Str => Try({}, [DirErr(IOErr)])
/// Refcounted fields are owned by the hosted function.
pub const DirCreate_allArgs = extern struct {
    arg0: RocStr,
};
...
/// Roc signature: Str => Try({}, [DirErr(IOErr)])
/// Refcounted fields are owned by the hosted function.
pub const DirDelete_allArgs = extern struct {
    arg0: RocStr,
};
...
/// Roc signature: Str => Try({}, [DirErr(IOErr)])
/// Refcounted fields are owned by the hosted function.
pub const DirDelete_emptyArgs = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const DirListArgs = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const EnvVarArgs = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const FileDeleteArgs = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const FileRead_bytesArgs = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const FileRead_utf8Args = extern struct {
    arg0: RocStr,
};
...
/// Refcounted fields are owned by the hosted function.
pub const FileWrite_bytesArgs = extern struct {
    arg0: RocStr,
...
/// Refcounted fields are owned by the hosted function.
pub const FileWrite_utf8Args = extern struct {
    arg0: RocStr,
    arg1: RocStr,
};
...
/// Roc signature: U64 => {}
/// Refcounted fields are owned by the hosted function.
pub const SleepMillisArgs = extern struct {
    arg0: u64,
};
...
pub const PlatformHostedFns = struct {
    cmd_host_exec_exit_code: *const fn (*RocOps, *Try, *const CmdHost_exec_exit_codeArgs) callconv(.c) void, // Cmd.host_exec_exit_code!
    cmd_host_exec_output: *const fn (*RocOps, *Try, *const CmdHost_exec_outputArgs) callconv(.c) void, // Cmd.host_exec_output!
    dir_create: *const fn (*RocOps, *Try, *const DirCreateArgs) callconv(.c) void, // Dir.create!
    dir_create_all: *const fn (*RocOps, *Try, *const DirCreate_allArgs) callconv(.c) void, // Dir.create_all!
    dir_delete_all: *const fn (*RocOps, *Try, *const DirDelete_allArgs) callconv(.c) void, // Dir.delete_all!
    dir_delete_empty: *const fn (*RocOps, *Try, *const DirDelete_emptyArgs) callconv(.c) void, // Dir.delete_empty!
    dir_list: *const fn (*RocOps, *Try, *const DirListArgs) callconv(.c) void, // Dir.list!
    env_cwd: *const fn (*RocOps, *Try, *anyopaque) callconv(.c) void, // Env.cwd!
    env_exe_path: *const fn (*RocOps, *Try, *anyopaque) callconv(.c) void, // Env.exe_path!
    env_var: *const fn (*RocOps, *Try, *const EnvVarArgs) callconv(.c) void, // Env.var!
    file_delete: *const fn (*RocOps, *Try, *const FileDeleteArgs) callconv(.c) void, // File.delete!
    file_read_bytes: *const fn (*RocOps, *Try, *const FileRead_bytesArgs) callconv(.c) void, // File.read_bytes!
    file_read_utf8: *const fn (*RocOps, *Try, *const FileRead_utf8Args) callconv(.c) void, // File.read_utf8!
    file_write_bytes: *const fn (*RocOps, *Try, *const FileWrite_bytesArgs) callconv(.c) void, // File.write_bytes!
    file_write_utf8: *const fn (*RocOps, *Try, *const FileWrite_utf8Args) callconv(.c) void, // File.write_utf8!
    random_seed_u32: *const fn (*RocOps, *Try, *anyopaque) callconv(.c) void, // Random.seed_u32!
    random_seed_u64: *const fn (*RocOps, *Try, *anyopaque) callconv(.c) void, // Random.seed_u64!
    sleep_millis: *const fn (*RocOps, *anyopaque, *const SleepMillisArgs) callconv(.c) void, // Sleep.millis!
    ...
    utc_now: *const fn (*RocOps, *u128, *anyopaque) callconv(.c) void, // Utc.now!
};
...
pub fn hostedFunctions(comptime fns: PlatformHostedFns) HostedFunctions {
    const Static = struct {
        const ptrs = [_]HostedFn{
            hostedFn(fns.cmd_host_exec_exit_code), // Cmd.host_exec_exit_code! (index 0)
            hostedFn(fns.cmd_host_exec_output), // Cmd.host_exec_output! (index 1)
            hostedFn(fns.dir_create), // Dir.create! (index 2)
            hostedFn(fns.dir_create_all), // Dir.create_all! (index 3)
            hostedFn(fns.dir_delete_all), // Dir.delete_all! (index 4)
            hostedFn(fns.dir_delete_empty), // Dir.delete_empty! (index 5)
            hostedFn(fns.dir_list), // Dir.list! (index 6)
            hostedFn(fns.env_cwd), // Env.cwd! (index 7)
            hostedFn(fns.env_exe_path), // Env.exe_path! (index 8)
            hostedFn(fns.env_var), // Env.var! (index 9)
            hostedFn(fns.file_delete), // File.delete! (index 10)
            hostedFn(fns.file_read_bytes), // File.read_bytes! (index 11)
            hostedFn(fns.file_read_utf8), // File.read_utf8! (index 12)
            hostedFn(fns.file_write_bytes), // File.write_bytes! (index 13)
            hostedFn(fns.file_write_utf8), // File.write_utf8! (index 14)
            hostedFn(fns.random_seed_u32), // Random.seed_u32! (index 15)
            hostedFn(fns.random_seed_u64), // Random.seed_u64! (index 16)
            hostedFn(fns.sleep_millis), // Sleep.millis! (index 17)
            hostedFn(fns.stderr_line), // Stderr.line! (index 18)
            hostedFn(fns.stdout_line), // Stdout.line! (index 19)
            hostedFn(fns.utc_now), // Utc.now! (index 20)
        };
    };
    return .{
...
pub extern fn roc__handle(ops: *RocOps, ret_ptr: *__AnonStruct70, arg_ptr: ?*const __AnonStruct69) callconv(.c) void;

/// Entrypoint: ws_for_host!
pub extern fn roc__ws_message(ops: *RocOps, ret_ptr: *__AnonStruct67, arg_ptr: ?*const __AnonStruct66) callconv(.c) void;
